(function (context) {
  ...
  var graph = new noflo.Graph('Window runtime for ' + component);
  graph.baseDir = 'hanaflo-runtime';
  ...
  function bindInports (inPorts) {
    ...
    context.addEventListener('message', function (message) {
      //later on check by whitelist
      /*if (message.origin !== origin) {
        return;
      }*/
      console.log("Received a message:"+JSON.stringify(message.data));
